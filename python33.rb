...
class Python33 < Formula
  ...
  skip_clean "bin/pip33", "bin/pip-#{VER}"
  skip_clean "bin/easy_install33", "bin/easy_install-#{VER}"
  ...
  def install
    ...
    # Demos and Tools
    (HOMEBREW_PREFIX/'share/python33').mkpath
    system "make", "frameworkinstallextras", "PYTHONAPPSDIR=#{share}/python33"
    system "make", "quicktest" if build.include? "quicktest"

    # Any .app get a " 3" attached, so it does not conflict with python 2.x.
    Dir.glob(prefix/"*.app").each do |app|
      mv app, app.gsub(".app", " 33.app")
    end
    ...
    # "python33" executable is forgotten for framework builds.
    # Make sure homebrew symlinks it to HOMEBREW_PREFIX/bin.
    ln_sf "#{bin}/python#{VER}", "#{bin}/python33"
    ln_sf "#{bin}/python#{VER}-config", "#{bin}/python33-config"
    ...
    resource('setuptools').stage { system "#{bin}/python33", *setup_args }
    mv bin/'easy_install', bin/'easy_install33'

    resource('pip').stage { system "#{bin}/python33", *setup_args }
    mv bin/'pip', bin/'pip33'
    rm bin/'pip3'
  ...
  def sitecustomize
    <<-EOF.undent
      ...
      if sys.version_info[0] != 3:
          ...
          opt_executable = '#{HOMEBREW_PREFIX}/opt/python33/bin/python#{VER}'
          if os.path.realpath(sys.executable) == os.path.realpath(opt_executable):
              ...
              try:
                  ...
                  build_time_vars['LINKFORSHARED'] = '-u _PyMac_Error #{HOMEBREW_PREFIX}/opt/python33/Frameworks/Python.framework/Versions/#{VER}/Python'
              except:
  ...
  def caveats
    text = <<-EOS.undent
      Setuptools and Pip have been installed. To update them
        pip33 install --upgrade setuptools
        pip33 install --upgrade pip

      You can install Python packages with
        `pip33 install <your_favorite_package>`
